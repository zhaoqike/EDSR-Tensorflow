...
class EDSR(object):
    ...
    def __init__(self,img_size=32,num_layers=32,feature_size=256,scale=2,output_channels=3):
        ...
        """
        mean = tf.constant([0.4488, 0.4371, 0.4040], dtype=tf.float32) * 255.0
        ...
        # x = tf.nn.conv2d(image_input, [3, 3, args.n_color, args.n_feats], padding='SAME')
        ...
        self.out = tf.clip_by_value(output+mean,0.0,255.0)
    ...
    def train(self,iterations=1000,save_dir="saved_models"):
        ...
        with self.sess as sess:
            ...
            for ep in range(num_epochs):
                it = DIV2K_iterator(div2k, batch_size=8)
                for i, j in enumerate(it):
                    print(i, len(j[0]), len(j[1]), it.now_index)
                    image_lrs = j[0]
                    image_hrs = j[1]
                    feed = {
                        self.input:image_lrs,
                        self.target:image_hrs
                    }
                    ...
                    if test_exists:
                        ...
                        #Write test summary
                        test_writer.add_summary(t_summary,ite)
